...
def train_one_epoch_distillation(teacher, student, criterion,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            teacher_prob = F.softmax(outputs_teacher / temp, dim=-1)
            student_prob = F.log_softmax(outputs / temp, dim=-1)
            distillation_loss = kl_loss(student_prob, teacher_prob) * (temp**2)
            ce_loss = criterion(outputs, targets)

            loss = alpha * distillation_loss + (1.0 - alpha) * ce_loss
            # *****************************************
        loss_value = loss.item()
...
def evaluate(data_loader, model, criterion, device):
    ...
    for images, target in metric_logger.log_every(data_loader, 650, header):
